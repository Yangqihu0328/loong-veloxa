...
Application::~Application() {
  ...
  // TASK-20260503-04 D.1 + D.2 — Console engine + buffer release first
  // (reverse of construction in LoadDevtoolDocument). Engine first because
  // its JS callbacks hold a JS_GetContextOpaque(ctx) pointer to the
  // buffer — clearing the engine first guarantees no callback can fire
  // against a freed buffer. See creative-devtool-console §C2.
#ifdef VX_BUILD_DEVTOOL
  console_script_engine_.reset();
  console_log_buffer_.reset();
#endif
...
void Application::UnloadDevtoolDocument() {
  // Reverse construction order: console engine → console buffer → devtool
  // bindings → devtool engine → update mgr → Document slot. Console engine
  // MUST go first so its JS callbacks (which hold buffer ptr via
  // JS_GetContextOpaque) cannot fire after the buffer is freed; the buffer
  // releases right after, before any other DevTool wires unwind. This is
  // the timing constraint documented in creative-devtool-console §C2.
  console_script_engine_.reset();
  console_log_buffer_.reset();
  devtool_dom_bindings_.reset();
  devtool_script_engine_.reset();
