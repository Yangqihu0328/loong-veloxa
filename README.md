...
[![ctest DEVTOOL=ON](https://img.shields.io/badge/ctest_DEVTOOL%3DON-1284_PASS-brightgreen)](#测试与基准)
[![ctest DEVTOOL=OFF](https://img.shields.io/badge/ctest_DEVTOOL%3DOFF-1091_PASS-brightgreen)](#测试与基准)
...
Veloxa 是一个为嵌入式 / 车载 HMI / IoT 显示设备打造的浏览器级渲染引擎。它实现了**子集 HTML5** + **CSS 引擎（~45 属性）** + **Block / Inline / Flex 布局** + **软件渲染器** + **W3C DOM 事件** + **CSS Transitions** + **QuickJS 脚本（含 Interrupt Handler T1 mitigation）** + **JS-DOM 绑定** + **完整自托管 DevTool 4 件套（Inspector + Performance Overlay + Hot Reload + Console JS REPL）**，通过稳定的 C ABI 集成到嵌入式应用，目标在 **100 MB 内存** 以内的硬件上流畅渲染 **60 fps** 的复杂界面。

> **MVP 范围参考：** 本项目采用 **三档分级 MVP-A/B/C** —— 详见 [`docs/specs/2026-05-04-mvp-scope.md`](docs/specs/2026-05-04-mvp-scope.md)。
...
Veloxa 内建一套**自托管（dogfood）** DevTool 4 件套 — 用 Veloxa 自身渲染调试 UI，无需第三方浏览器即可对目标 Document 做 Inspect / 性能监控 / 热重载 / Console JS REPL eval。
...
| **D** | Console JS REPL | Isolated JSRuntime + capability allowlist + console.log/warn/error bridge + T1 5 维度 mitigation + ` hotkey toggle | `vx_devtool_console_eval` / `vx_devtool_console_get_log_buffer` |
...
# F12 切换 DevTool 可见 / F11 切换 Performance HUD / ` 切换 Console JS REPL
```

### 安全设计

- **A14 链接闭包零字节增长** — `VX_BUILD_DEVTOOL=OFF` 时二进制体积零增长（pre-merge ctest smoke 强制守门）
- **T1 任意 eval mitigation 5 维度** — Console JS REPL Isolated JSRuntime + capability allowlist + Interrupt Handler 执行预算 + RuntimeMemoryLimit 32 MiB + dual-cap log buffer
...
| 配置 | 测试数 | 耗时 |
|---|:-:|:-:|
| `DEVTOOL=ON` | **1284 PASS** | ~6.4s |
| `DEVTOOL=OFF` | **1091 PASS** | ~6.4s |
| `SDL2=ON` | **~1300 PASS** | ~6.5s |

**所有配置零退化** — A14 链接闭包 + ctest smoke 强制守门 + commit body Source 段 baseline 强制守门（[creative-mvp-A.md §6.1](memory-bank/creative/creative-mvp-A.md)）。
...
### MVP 档对照（详见 [`docs/specs/2026-05-04-mvp-scope.md`](docs/specs/2026-05-04-mvp-scope.md)）

| MVP 档 | 完成度 | 代表性场景 | 已知 gap 数 | 估时 plan ×0.6（达成下一档）|
|:-:|:-:|---|:-:|:-:|
| **MVP-A 最小可运行 demo** | ✅ **100%** | 嵌入式 boot 屏 / 简单状态显示 | **0** | — |
| **MVP-B 桌面 dogfood 完整** | ⚠️ **~90%** | HMI 原型开发 / Demo 演示 / 第三方桌面 evaluation | **4** | ~2.5-6 h |
| **MVP-C 真嵌入式部署** | 🟡 **~70%** | 车载中控 / 仪表盘 / 工业 HMI / IoT 面板 / 医疗设备 | **9** | ~40-80 h（核心 P0）|
...
| 2026-05-03 | **DevTool Phase D · Console JS REPL** ✅ | Isolated JSRuntime + capability allowlist + console.log/warn/error bridge + T1 5 维度 mitigation + ` hotkey toggle / **DevTool 4 件套全闭环** 🎉 |
| 2026-05-04 | **MVP 范围定义蓝图（V1=D 三档分级）** | spec + plan + creative ×3（MVP-A/B/C 各一篇）|

### 路线图（按 MVP 档分层）

#### 短期（MVP-B 收口 / ~2.5-6 h plan ×0.6）

- ⭐ **DomBindings R2 三连补全**（`Element.children` / `addEventListener` / `innerHTML setter`）— **第一推荐 / 解锁第三方桌面 evaluation 完整 dogfood 视觉**
- **Performance Overlay 持续 invalidate 机制** — dirty rect 多帧滑动可见

#### 中期（MVP-B → MVP-C 过渡 / ~5-12 h plan ×0.6）

- **资源加载策略蓝图**（systemPatterns 「待定架构决策」清零）
- **R9 EventManager HitTest 改造** — HUD pointer-events 真支持

#### 长期主线（MVP-C 核心 P0 / ~40-80 h plan ×0.6）

- 🌟 **G1 OpenGL ES 硬件渲染后端**（核心目标 #2 硬指标 / Level 4 多 Phase）
- 🌟 **G2 DRM/KMS 嵌入式 Linux 后端**（核心目标 #1 主要平台硬指标）

#### 长期次线（MVP-C 其他 gap）

- DOM 节点动态创建删除（`createElement` / `removeChild` / TextNode）
- CSS 高级 5 项（per-corner border-radius / `@keyframes` / z-index stacking / `position: fixed` 真支持 / `pointer-events: none` 真支持）
- 图像扩展 3 项（WebP / 双线性缩放 / 异步图片加载）
- 性能优化收口（#35 阶段 2 拆 LayoutEngine / R3+ 13 项 P1 候选）
...
- Vulkan 渲染后端 / 完整 Web Animations API / SVG / 完整 ICU 国际化

详细 P3 候选清单见 [`memory-bank/activeContext.md`](memory-bank/activeContext.md) 「待处理事项」段；详细 MVP 范围定义见 [`docs/specs/2026-05-04-mvp-scope.md`](docs/specs/2026-05-04-mvp-scope.md)。
