...
20. Block 布局缺少 margin collapsing（CSS 规范要求，影响渲染正确性）— 🟢 **TASK-20260426-01 R3 部分修复**（W3C CSS 2.1 §8.3.1 sibling collapse / collapse-through / negative max(pos)+min(neg) / BFC root 阻断 4 类规则完整实施；first/last child 与 parent collapse + clearance 受 D1.2 LayoutChild API 边界约束 stub-w/-rationale 留 P3 TASK-26-02；新文件 `veloxa/core/layout/margin_collapse.h` POD `MarginChain` header-only inline + `Add/Collapsed/IsEmpty/ApplyClearance/Trace`；`layout_engine.cc::LayoutBlock` 重写 + 3 helper inline `IsInFlow`/`CreatesBlockFormattingContext`/`IsCollapseThrough`；`layout_box.h` 增 2 状态字段 `collapsed_through` + `margin_top_collapsed_into_ancestor`；ctest +26 cases = 1010/1010 PASS（11 MarginChain unit + 11 MarginCollapseLayoutTest 集成 + 4 wpt 数值 2 PASS 002/003 + 2 SKIP-w/-rationale 001/005）；bench `BM_LayoutBuildTreeFlat/64` +10.2% 边缘超 +10% 阈值（中位 4087 ns vs baseline 3709 ns，全维 +13~19% 真实算法代价非 noise，每 box 增 5-7 ns × 64 = 320-450 ns 与观测吻合）；副产品修 1 pre-existing bug `style_resolver.cc` `overflow: auto` 经 ParseDeclaration 走 `ValueType::kAuto` 全局快路径不进 ParseEnumValue 致 enum 字段保持 default kVisible — 由 R3.4 `A7_OverflowAutoBlocksMarginCollapse` 测试触发发现）
