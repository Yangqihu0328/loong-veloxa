...
|---|---|---|---|
| **蓝图任务 V2=a + 批量决策跳过 + 批量文档（≥ 3 篇）** | **0.21-0.59× ×0.6 / 0.20-0.38× plan** | **TASK-30-04 (0.27-0.35× plan / 0.46-0.59× ×0.6) + TASK-30-02 (0.22× plan) + TASK-30-01 P6 (0.21× plan) + TASK-20260504-01 (0.25-0.38× plan / standalone-AI) — 4 数据点群组** | **极窄档 — 取下限保守估，加 ~30% buffer 给 reflect/archive；TASK-20260504-01 standalone-AI 实测条件下沿延伸到 0.25× plan** |
...
| **纯文档/规则极速区**（新子档 — TASK-20260503-02 反思入库）| **0.15-0.25× ×0.6** | **TASK-20260503-02 工作流元任务 6 子任务（0.21× 群组）— 第 56-61 数据点** | **触发条件叠加（4 项必须全满足）：(1) 任务类型为纯文档/规则/spec 修改**（无新代码逻辑变更 / 修改文件 .mdc/.md/spec.md）**+ (2) 零 build 风险**（无 link / 无新依赖 / 无 CMake 变更 / 无 RED→GREEN cycle）**+ (3) 验收方式为 git diff 可见 + ctest 不退化**（无新单测）**+ (4) Phase 0 audit 预跑模式**（如有 audit 子任务，结果必须在 Phase 0 阶段固化到 plan 文档）— 实测 6/6 子任务全部命中 0.15-0.50× 区间，总比值 0.21× 远超极窄档加速衰减区下沿；加速因子：无 RED→GREEN / 无 ctest 等待 / 无 link 风险 / 同文件 batch 收益 / Phase 0 audit 预跑（任务 5 仅文档化）/ StrReplace 工具天然 hunk 隔离（path-level isolation 等价 `git add -p`）|
| **最小代码改动 + Phase 0 高度预跑极速区**（新子档 — TASK-20260503-05 反思入库）| **0.10-0.20× ×0.6** | **TASK-20260503-05 QuickJS Interrupt Handler 5 子任务 + 1 finalize（0.16× 群组）— 第 67-72 数据点** | **触发条件叠加（5 项必须全满足）：(1) 真实代码改动 ≤ 200 行**（本任务 +184 行 / 5 文件）**+ (2) Phase 0 grep audit 完整 ≥ 3 grep + 关键既有文件 audit**（如 Status.h enum audit）**+ (3) 完整 cpp 代码片段 plan 阶段 100% 就位**（含注释 / namespace / public-private 顺序 / constexpr / static 等所有细节，可直接 Write 编译）**+ (4) 0 brainstorm 之外的设计决策**（brainstorm + B 决策表全锁死，build 阶段无新设计）**+ (5) 测试矩阵预 audit（含 ctest config guard 假设）**（注意：本任务此项有 plan-fact reconcile #1 — DEVTOOL=OFF 测数预测错误，已沉淀 P1 改进 writing-plans audit 子条强化）— 实测 5/5 实施子任务命中 0.08-0.30× 区间，总比值 0.16× 远超「纯文档/规则极速区 0.15-0.25×」下沿但**加速因子不同**：本子档**有真实 cpp 代码 + 单测 RED-GREEN cycle**（vs 纯文档子档无 build 风险）；加速因子：完整 cpp 片段 plan 就位 + Phase 0 grep audit 全 surface 重大设计偏差（D8b 避免 100-1000s 死循环灾难）+ Status.h audit 提前发现 + Level 2 + creative 已预决策 = 0 设计探索成本 |
| **creative 全锁死 + 范式 100% 复用 + AI agent 极速区**（新子档 — TASK-20260503-04 反思入库 / 含 AI agent 加速因子标注 ⚠️）| **0.05-0.15× ×0.6** | **TASK-20260503-04 DevTool Phase D Console JS REPL 5 子任务 + finalize（0.07-0.10× 群组）— 第 73-77 数据点 / 实测条件：standalone-AI** | **触发条件叠加（6 项必须全满足）：(1) Creative 阶段 4+ 维度全锁死（含完整 cpp 代码骨架）**（本任务 C1-C4 4 维度 + 1197 行 creative 文档全 cpp 可编译片段）**+ (2) 5 大架构范式 + 9 systemPatterns ≥ 95% 命中（含 N/A 计入）**（本任务 100% 命中 + 2 项新沉淀候选 — isolated JSRuntime + capability allowlist）**+ (3) Phase 0 grep ≥ 7 子段实证**（前置依赖 / runtime 行为 / config 矩阵 / API 能力 / 反复模式抑制）**+ (4) 0 brainstorm 之外的设计决策**（C1-C4 + B1-B8 = 12/12 决策锁死）**+ (5) plan-fact reconcile ≤ 2 项 + 在 build 同任务内修复**（无返回 /creative 重做 / 0 重写代价 — 通过既有范式扩展自动修复）**+ (6) AI agent 加速因子（standalone-AI 实测条件）**（Edit/Write 接近瞬时 + ctest 27s 全跑 + 工具调用并行）— 实测 5/5 实施子任务命中 0.05-0.13× 区间，总比值 0.07-0.10× 创历史新低（破 TASK-05 0.16×）；**范围远大于「最小代码改动」子档**：含 +2181 行 src+tests / 32 测 / 6 commits / plan-fact reconcile 双 / DEVTOOL 双 config — 须建立独立新子档 + AI agent 实测条件列。**⚠️ 重要标注**：本子档实测含强 AI agent 加速因子，**应明确区分「AI agent 实测」vs「人类开发实测」**，避免后续任务用此基线给人类开发预算造成误导（详 §AI agent 实测条件标注协议）|
...
- **TASK-20260503-02（第 56-61 数据点入库 = 6 个新数据点群组）** — 工作流元任务（清理跨任务 reflection §6/§7 沉淀）Level 2 任务，~18 min 主线 vs ~85 min plan ×0.6 = **0.21×（落「纯文档/规则极速区」新子档 0.15-0.25×）**：
  - 6 子任务分布：C-#1 0.33-0.50× / C-#2 0.20× / C-#4 0.20× / A-P1#4 0.20-0.30× / A-P1#6 0.15-0.20× / A-P1#8 0.15-0.20×
  - **总比值 0.21× 创历史新低**（vs Phase C 0.31× 进一步加速 32%）；中位 0.20×，6/6 子任务全部 ≤ 0.50×
  - **反复模式命中 0/7**（创纪录第三次连续零反复 — Phase A → B → C → 本任务）
  - **Phase 0 audit 预跑模式首次实证扩展 Phase 0 定律应用边界**（quad-evidence 升级：A 5.3× / B 5.2× / C 7.6× / **本任务 ~7-9× ROI**）— 任务 5 audit 文档化耗时 ~3 min（vs plan 15-20 min），关键加速因子是 plan §0.2 阶段直接预跑 grep audit 命令并固化结果
  - **C-#2 反复模式渐进式抑制首次实证有效**：第二次同类反复（来自 TASK-20260502-02 P1 #2）→ 在 plan + commit 消息 + 自我吃狗粮三层抑制 → 阻断进入「3 次反复 = P0」轨道
  - **新协议首次实证 4 项**：(1) reflection 沉淀回流模式（4 from C + 3 from A 项 P1 跨任务批量清零）+ (2) 反复模式渐进式抑制 + (3) Phase 0 audit 预跑模式 + (4) 纯文档/规则极速区识别
  - VAN/plan 预测 0.40-0.50×（极窄档延续假设）— 实测 0.21× 远低于预测 50%；预测应在「纯文档/规则」+「Phase 0 audit 预跑」+「同文件 batch」三因素叠加时下调到 0.15-0.25× 区间
- **TASK-20260503-05（第 67-72 数据点入库 = 6 个新数据点群组）** — QuickJS Interrupt Handler + SetEvalInterruptBudget API（技术债 #44 组件 1 Phase 2 闭环）Level 2 任务，~15 min 主线 vs ~85-105 min plan ×0.6 = **0.16×（落「最小代码改动 + Phase 0 高度预跑极速区」新子档 0.10-0.20×）**：
  - 6 子任务分布：E1 0.20× / E2 0.08-0.10× / E3 0.08-0.10× / CP1（含 build-off ~60s）~3 min / E4 0.20× / E5 0.30×
  - **总比值 0.16× 创历史新低**（vs TASK-20260503-02 0.21× 进一步加速 24%）；中位 0.16×，5/5 实施子任务全部 ≤ 0.30×
  - **反复模式命中 0/7**（连续第 5 次零反复达成 — Phase A → B → C → 02 → 03 [1/7 回升] → **本任务抵消** Phase 0 §0.3 三 grep + §0.3.4 Status.h audit + D8b push-back **三层抑制全部生效**）
  - **Phase 0 投入定律 sext-evidence 升级**（5.2-16× ROI / 平均 8.1×）：A 5.3× / B 5.2× / C 7.6× / 02 6.7× / 03 8.0× / **05 16×（创历史新高）** — 16× 加速因子：(1) Phase 0 极简 1 子段（5 min）vs 历史 30 min（Level 3）+ (2) 4 grep 实证全 surface 重大设计偏差（**D8b 重校准默认 budget 避免 100-1000s 死循环灾难**）+ (3) Status.h audit 提前发现避免 build 阶段 enum 缺失返工 + (4) Level 2 + creative 已预决策 = 0 设计探索成本
  - **新协议首次实证「Phase 0 grep 实证驱动的主动 push-back 模式」（D8b 实证）**：brainstorm scope 已被 core_only 限定后，Phase 0 §0.3.2 grep 实证发现 `JS_INTERRUPT_COUNTER_INIT = 10000`（quickjs.c:474-476）→ 主动抛出 D8b（不在用户原始 D 选项）→ 重校准 creative「10⁷ 检查点」字面值（避免 10¹¹ 字节码 ≈ 100-1000s 死循环）；触发条件清单（3 项）：(a) brainstorm scope 已被用户限定 + (b) Phase 0 grep / audit 阶段发现 creative / spec / 既有实现 runtime 行为偏差 + (c) 偏差**显著**（默认值差 10³+ 倍 / API 不存在 / signature 不符）→ **必须主动**抛出而非等用户问到（「证据优于断言」原则的延伸）— 已沉淀 P1 改进建议到 `.cursor/rules/skills/brainstorming.mdc` 新段（迁移 activeContext 待处理事项 / 下次工作流元任务批量落地）
  - **plan-fact reconcile #1（反复模式 #1 第 3 个新形式）**：plan §0.4 假设 DEVTOOL=OFF 1082 不变错误 / 实际 1087（+5 — quickjs_engine_test 在 vx_script 无 DEVTOOL guard）；这是反复模式 #1「前置依赖/环境/API 能力未验证」的**第 3 个新形式**「config 矩阵 guard 边界假设漏审」（vs 历史 9 次「依赖包/工具链是否存在」+ TASK-03 第 9 次「现有实现 runtime 行为假设」）— CP1 自审已捕获 + 沉淀 P1 改进建议到 `.cursor/rules/skills/writing-plans.mdc` «ctest 数量预期 config 矩阵» 段补强 audit 子条 + grep 范本（已迁移 activeContext 待处理事项）
  - VAN/plan 预测 0.30-0.45×（极窄档延续高效区候选）— 实测 0.16× 远低于预测 50%；预测应在「最小代码改动 + Phase 0 高度预跑」5 触发条件叠加时下调到 0.10-0.20× 区间
  - **commit body Source 溯源 + 实测数据格式 triple-evidence 升级**：TASK-20260503-02（6 docs commits）+ TASK-20260503-03（5 commits）+ **TASK-20260503-05（5 实施 commits + plan + reflect + archive = 8 commits）** = **19 commits 累计 triple-evidence**（达成 git-workflow.mdc 固化阈值，下次工作流元任务批量落地时同步固化）
- **TASK-20260503-04（第 73-77 数据点入库 = 5 个新数据点群组 + 实测条件：standalone-AI）** — DevTool Phase D · Console JS REPL + console.log 桥接（V1=B 扩展段 / spec §11.1 完整闭环）Level 3 安全相关任务，~17 min build phase 主线 vs 180-240 min plan ×0.6 = **0.07-0.10×（落「creative 全锁死 + 范式 100% 复用 + AI agent 极速区」新子档 0.05-0.15× / 创历史新低破 TASK-05 0.16×）**：
  - 5 子任务分布：D.1 0.08-0.10× / D.2 0.06-0.08× / D.3 0.06-0.08× / D.4 0.10-0.13× / D.5 0.05-0.08× / finalize ~1 min
  - **总比值 0.07-0.10× 创历史新低**（vs TASK-20260503-05 0.16× 进一步加速 ~50%）；中位 0.07-0.08×，5/5 实施子任务全部 ≤ 0.13×
  - **实测含强 AI agent 加速因子**（standalone-AI 实测条件）：Edit/Write 接近瞬时 + ctest 27s 全跑 + 工具调用并行；**应在新子档明确区分「AI agent 实测」vs「人类开发实测」**，避免后续任务用此基线给人类开发预算造成误导（详 §AI agent 实测条件标注协议 — 紧随本段 ↓）
  - **范围远大于 TASK-05「最小代码改动」子档**：含 +2181 行 src+tests / 32 测 / 6 commits / plan-fact reconcile 双 / DEVTOOL 双 config — 须建立独立新子档「creative 全锁死 + 范式 100% 复用 + AI agent 极速区」0.05-0.15× ×0.6（vs 「最小代码改动」0.10-0.20× — 加速因子不同：本子档**范式复用 + creative 全锁死**主导，vs 「最小代码改动」**代码量极简**主导）
  - **反复模式命中 1/7 部分命中**（反复模式 #1 第 11 次抑制成功部分 — main runtime 已抑制 / 2 个新形式漏审：panel JS ctx 归属 + config 矩阵 guard 边界假设）— 触发**反复模式 #1 子维度「config 矩阵 guard 边界假设漏审」第 2 次连续命中**（TASK-05 第 1 次 → TASK-04 第 2 次）→ **达 P0 升级阈值**（systemPatterns「反复模式渐进式抑制」段 3 次=P0 原则）→ archive 阶段已立即落实 writing-plans.mdc «add_test config guard 边界假设审计» 强制 audit 子条（不再等下次工作流元任务批量落地）
  - **plan-fact reconcile 双命中 + 0 重写代价**：(1) C2 wiring（panel JS 实际跑在 devtool_script_engine ctx 而非 console_script_engine ctx）→ D.4 通过 DomBindings host data channel 扩展（复用 SetHotReloadManager 范式 / +150 行）修复 / 7 测一次过 / 0 返回 creative；(2) OFF +4 而非 +0（console_api_test 含 OFF stub 测路径不加 add_test guard 让 OFF 也跑）→ 已升级 P0 改进
  - **Phase 0 投入定律 sept-evidence 升级**（5.2-16× ROI / 平均 ~8.4×）：A 5.3× / B 5.2× / C 7.6× / 02 6.7× / 03 8.0× / 05 16× / **04 ~6-10× 估值** — 7 次实证 sept-evidence
  - **2 项新沉淀候选**：(1) **DevTool isolated JSRuntime 协议**（spec §11.1 落地范本 — 第 3 个 QuickjsEngine 实例 + console_log_buffer 隔离 + lifecycle 反序释放 + capability allowlist 严格隔离 + DomBindings host data channel 扩展作为 panel JS dual-ctx wiring 解决方案）→ archive 阶段已立即沉淀（详「DevTool isolated JSRuntime 协议」段）；(2) **plan-fact reconcile build 阶段同任务内修复透明化协议** — commit message Source 段明确标注「plan-fact reconcile #N」+ techContext 实施摘要 §显式记录（vs 沉默修复后续审计困难）
  - VAN/plan 预测 0.30-0.45×（极窄档延续高效区候选）— 实测 0.07-0.10× 远低于预测 70%；预测应在「creative 全锁死 + 范式 100% 复用 + AI agent」6 触发条件叠加时下调到 0.05-0.15× 区间（standalone-AI 实测条件）
  - **commit body Source 溯源 + 实测数据格式 quad-evidence 升级**：TASK-20260503-02（6 docs commits）+ TASK-20260503-03（5 commits）+ TASK-20260503-05（5 实施 + plan + reflect + archive = 8 commits）+ **TASK-20260503-04（5 实施 + finalize + plan + creative + reflect = 9 commits / 待 archive +1 = 10）** = **27-28 commits 累计 quad-evidence**（**远破 git-workflow.mdc 固化阈值**，下次工作流元任务批量落地时强制固化）
- **TASK-20260504-01（第 78-83 数据点入库 = 6 个新数据点群组 + 实测条件：standalone-AI / 蓝图 V2=a 第 4 数据点）** — MVP-scope 文档蓝图任务 Level 4 V2=a 完整变体，~45 min 主线（除用户中断）vs 120-180 min plan ×0.6 = **0.25-0.38×（落「蓝图任务 V2=a + 批量决策跳过 + 批量文档（≥ 3 篇）」子档下沿延伸）**：
  - 5 commits 子任务分布：commit 1 plan+spec ~17 min（plan 25min × 0.68×）/ commit 2 creative-A ~3 min / commit 3 creative-B ~5 min / commit 4 creative-C ~9 min / commit 5 finalize ~11 min / + commit 6 reflect ~10 min = 总 standalone-AI ~55 min
  - **总比值 0.25-0.38×**（落入 plan §5 estimating 上限预测中上区间），**第 4 数据点入库** + 形成「蓝图任务 V2=a」子档的下沿延伸（之前 0.33-0.59× → 现 0.21-0.59×）
  - **反复模式命中 0/7**（创第 6 次连续 0 反复纪录候选 — Phase A → B → C → 02 → 05 → 04 → **本任务**）+ **1 项新候选首次定型**（中文文档 StrReplace 全角/半角字符差异 — 已 reflect 阶段立即沉淀 P0 mitigation）
  - **Phase 0 投入定律 sept-evidence 第 8 次实证**：A 5.3× / B 5.2× / C 7.6× / 02 6.7× / 03 8.0× / 05 16× / 04 ~6-10× / **本任务 ~3-5×（蓝图任务范围较小 / 节省风险偏小 ROI 偏低 / 范式延续完整）** — 8 次实证 sept-evidence 维持
  - **跨决策协同度 100% 第 8 次连续命中**：11/11（V1+V3+V4+V5+B1-B7）1 次 AskQuestion all_recommended 锁定 / 累计 11+12+12+10+13+12+12+11 = **93/93 跨决策一次锁定纪录** / 已 archive 阶段沉淀新段「跨决策协同度 100% sept-evidence」
  - **3 项新沉淀 archive 阶段直接落实**：(1) 蓝图任务规模估算公式（systemPatterns 新段 / 6 数据点公式 / spec 12 段×35 + 4 附录×30 ≈ 540 行预测精度）+ (2) 超 MVP plus 标识范式（systemPatterns 新段 / 解决 DevTool Phase E/F/G 优先级争议 / 可推广到 Vulkan/SVG/Web Animations）+ (3) 跨决策协同度 100% sept-evidence（systemPatterns 新段 / 8 次连续命中 / 93/93 累计）
  - VAN/plan 预测 0.14-0.40×（极宽 — 因蓝图任务无 build phase 风险）— 实测 0.25-0.38× **完美落入预测中上区间**
  - **commit body Source 溯源 + 实测数据格式 quad-evidence 升级**：TASK-20260504-01（5 实施 + reflect + archive = 7 commits）= **34-35 commits 累计 quad-evidence**（远破 git-workflow.mdc 固化阈值）

### AI agent 实测条件标注协议（TASK-20260503-04 反思入库 / 与 plan ×0.6 矩阵配套必填）

> **背景**：TASK-20260503-04 实测 build phase ~17 min vs plan ×0.6 180-240 min = 0.07-0.10× 创历史新低，含强 AI agent 加速因子（Edit/Write 接近瞬时 + ctest 27s 跑 + 工具调用并行）；如不区分实测条件，后续任务用此基线给人类开发预算会造成严重误导（人类开发预期可能 ~3-5× AI agent 实测时长）。

**强制标注协议**（plan ×0.6 矩阵段每个数据点群组必填）：

| 实测条件 | 适用场景 | 加速因子参考 |
|---|---|:-:|
| **standalone-AI** | AI agent 单独完成（无人类介入）/ Edit/Write 工具瞬时编辑 / ctest 27s 跑 / 工具调用并行 | ~3-10× vs human-only |
| **human-only** | 纯人类开发 / 手动编辑 / 手动跑测 / 手动验证 | 1.0× 基线 |
| **mixed** | AI 主导 + 人类决策 + 偶尔手动验证 | ~1.5-3× vs human-only |

**plan ×0.6 数据点群组实测条件分布**（截至 TASK-20260503-04）：

| TASK | 比值 | 实测条件 | 备注 |
|---|:-:|---|---|
| 502-01 Phase A | 0.64× | mixed | escalation 触发人类决策 |
| 502-02 Phase B | 0.40× | mixed | 混合 AI + 人类验证 |
| 503-01 Phase C | 0.31× | mixed | 范式复用 + AI 加速 |
| 503-02 工作流元 | 0.21× | standalone-AI | 纯文档 / AI 主导 |
| 503-03 三件套收官 | 0.42× | mixed | P1 失败回退 + 人类决策 |
| 503-05 Interrupt | 0.16× | standalone-AI | 极简 5 子任务 / AI 主导 |
| **503-04 Console** | **0.07-0.10×** | **standalone-AI** | **纯 AI agent / Edit/Write 瞬时 / ctest 27s 全跑并行** |

**判读规则**：

- standalone-AI 比值不应直接用于 human-only 估时 — 推荐除 ~3-5× 还原到人类基线
- mixed 比值可作 mixed 类任务参考基线
- human-only 比值应作历史早期任务参考基线（TASK-20260405 之前的任务）

**沉淀方式**：本协议强制要求 plan ×0.6 数据点群组实测条件标注列；reflect 阶段应在「估时校准」段标注实测条件 + 折算 human-only 估值（如 standalone-AI ~17 min → 人类预期 ~50-85 min）。
...
## reflection 沉淀回流模式（TASK-20260503-02 反思 §5.3 首次实证）

### 背景

每次 reflection §6 改进建议会产生 P1/P2 项，迁移到 `activeContext.md` 待处理事项段。多次任务后，P1 项逐渐累积（Phase A reflection §6 沉淀 3 项 P1 + Phase C reflection §7 沉淀 4 项 P1）。如果不主动清零，会发生：

1. **「3 次反复 = P0 升级」轨道触发** — 同类 P1 多次出现升级为系统性问题
2. **「待处理事项漂移」反模式** — 列表越长越容易被忽略
3. **跨任务上下文丢失** — 旧 P1 的 source / 上下文被新任务覆盖

### 协议

**触发条件（满足任一）：**

- 累积 ≥ 4-6 项跨任务 P1 待处理事项 + 项之间无强依赖
- 同类反复模式第二次出现（升 P1 警戒线后必须主动清零）
- 当前无紧急功能任务（适合穿插「工作流元任务」做维护）

**任务定位：「工作流元任务」**（vs 实施类任务 + 蓝图类任务）

| 类型 | 定义 | 估时区间 | 实证 |
|---|---|:-:|---|
| 实施类任务 | 含新代码逻辑（features / fixes / refactors） | 0.30-0.65× plan ×0.6 | Phase A/B/C |
| 蓝图类任务 | spec + plan + creative ×N，无 build 实施 | 0.20-0.35× plan ×0.6 | TASK-30-04 |
| **工作流元任务** | **清理跨任务 reflection 沉淀 / 规则调整 / 文档更新** | **0.15-0.25× plan ×0.6** | **TASK-20260503-02 首次实证** |

### 标准流程

```
1. /van 阶段：用户描述「清理技术债」/「整理工作流」等触发词 → AskQuestion 列出累积 P1 池
2. /plan 阶段：B 级精化决策 8 项典型 — 包括 commit 粒度（多 commits/ID）+ 测试模式（[文档调整模式]）+ Phase 0 audit 预跑
3. /build 阶段：按 B3 锁定顺序执行（同文件 batch 收益 + Phase 0 audit 预跑加速）+ CP 自审主动质疑范围
4. /reflect 阶段：识别本任务自身落实的 P1/P2 → archive 阶段直接落实到 systemPatterns
5. /archive 阶段：将本任务落实的 P1 在「待处理事项」段标 ✅ 已落实 + 闭环描述（含 commit hash）
```

### 收益

- **避免「3 次反复 = P0」轨道触发** — TASK-20260503-02 抑制 C-#2 第二次反复（来自 Phase B P1 #2）+ 闭环 Phase A P1 #4/#6/#8 + Phase C P1 #1/#2/#4
- **「待处理事项段」长度可控** — 7 项 P1 一次性清零，避免漂移
- **跨任务 source 链可追溯** — 6 docs commits 全部含「Source: TASK-XXXXXXXX-XX reflection §X」前置溯源（自发新 commit message convention）
- **极速完成** — 0.21× 总比值（plan ×0.6 第 56-61 数据点群组实证）

### 反模式

- ❌ 等到累积 ≥ 10 项 P1 才清理 — 列表过长难以一次性处理 + 上下文老化
- ❌ 把工作流元任务与实施类任务捆绑（如「Phase D 顺便清理 P1」）— 估时模型不同导致错配
- ❌ 工作流元任务不做 reflection / archive 闭环 — 失去 source 链 + 失去新协议沉淀机会

### 交叉引用

- `memory-bank/systemPatterns.md`「plan ×0.6 矩阵 — 纯文档/规则极速区 0.15-0.25×」段（同源协议数据点）
- `memory-bank/systemPatterns.md`「反复模式渐进式抑制」段（C-#2 第二次抑制实证）
- `memory-bank/reflection/reflection-TASK-20260503-02.md`（首次实证完整反思）

---

## Phase 0 投入越深 / build phase 越快定律（TASK-20260502-02 反思 §2.1 — dual-evidence / TASK-20260503-01 反思 §6.1 — triple-evidence / TASK-20260503-02 反思 §5.1 — quad-evidence 升级 + audit 子模式）
...
**TASK-20260503-02 工作流元任务**（第四次实证 — quad-evidence 升级 + audit 子模式首次实证）：
- Phase 0 实测投入 ~10 min（极简 1 子段 — 工具链快照 + audit 预跑 + 工具可用性矩阵）
- build phase 实测 18 min vs plan ×0.6 85 min = 0.21× → 节省 67 min
- ROI = 67 / 10 = **6.7×**
- **首次扩展 Phase 0 投入定律应用边界 — audit 子模式**：plan §0.2 直接预跑 A-P1#6 的 grep audit（全 codebase `.status()` 调用）固化 6/6 ✅ 结果到 plan 文档 → 任务 5（audit 文档化）耗时 ~3 min（vs plan 15-20 min）= 0.15-0.20× 加速。**audit 类任务的 Phase 0 预跑模式应用条件**：
  - audit 命令可单次执行（不依赖 build artifacts）
  - audit 结果可文档化（不需运行时 fix 决策）
  - audit 范围可枚举（如 `grep` / `find` 等）
- **不适用边界**：性能 benchmark audit（需独立 build-bench 环境）/ 安全漏洞 audit（需 SAST 工具链）
...
| TASK-20260503-02 工作流元 | 10 min | 85 min | 18 min | 67 min | **6.7×** |
| **平均** | 25 min | 280 min | 127 min | 153 min | **6.2×** |

**结论：** ROI 区间 5.2-7.6×（quad-evidence），平均 6.2×。**ROI 不是固定值，而是随「跨任务范式累积」呈正反馈增长**：
...
- TASK-20260503-02 沉淀「audit 子模式」+「纯文档/规则极速区」+「reflection 沉淀回流模式」3 项新协议 → audit 类任务 + 工作流元任务可直接套用
...
## DEVTOOL=OFF / 次配置 baseline 验证 SOP — FETCHCONTENT_BASE_DIR 复用范式（TASK-20260503-05 反思 §4.3 — 新模式 / 首次实证）

### 背景

CP1 / CP2 自审需要在多个 build config（`DEVTOOL=ON` 主 build/ + `DEVTOOL=OFF` build-off/ + 必要时 `SDL2=ON` build-sdl2/）下验证 ctest baseline 不退化。问题：每个新 build 目录冷启动 cmake configure 时，`FetchContent` 会重新拉取所有外部依赖（quickjs-ng / googletest / freetype / harfbuzz / libpng / libjpeg / 等），耗时 ~75s+，严重拖慢次配置 baseline 验证。

### 触发条件

- 主 build 目录（如 `build/` DEVTOOL=ON）已完整配置 + `_deps/` 已 FetchContent 完成
- 需要新建次配置 build 目录（如 `build-off/` DEVTOOL=OFF / `build-sdl2/` SDL2=ON）做 baseline 验证
- 次配置 build 目录对依赖版本/补丁完全相同（仅 cmake option 差异，外部依赖集合一致）
...
# Configuring done (1.8s) — 复用主 build/ 的 _deps，不重新拉取
cmake --build build-off  # ~60s 全量增量构建（首次）
ctest --test-dir build-off  # baseline 验证
```

### 关键设计点

1. **`-DFETCHCONTENT_BASE_DIR` CMake 变量** — 指定 FetchContent 寻找/缓存依赖的根目录，复用主 build/ 的 `_deps/` 子树
2. **绝对路径必需** — `$(pwd)/build/_deps` 或 `$ENV{HOME}/code/<repo>/build/_deps`，相对路径会被 CMake 在不同 source-out 目录解析为不一致路径
3. **依赖一致性前提** — 次配置必须使用与主 build 相同的依赖版本（不可在次 build 启用「升级 quickjs-ng 到 master」等会改变 `_deps/` 内容的 option）
4. **可结合 `FETCHCONTENT_FULLY_DISCONNECTED=ON`** — 确保 cmake configure 阶段完全不联网（避免 _deps 已存在但仍触发网络检查）

### 收益数据

| 阶段 | 冷启动 | FETCHCONTENT_BASE_DIR 复用 | 节省 |
|---|:-:|:-:|:-:|
| cmake configure | ~75s+（FetchContent 拉取所有依赖） | **1.8s**（仅 generate） | **~73s** |
| 全量构建 | ~60s | ~60s | 0（构建本身不可省） |
| 总 baseline 验证 | ~135s+ | **~70s** | **~65s（48% 加速）** |

### 反模式

❌ **「直接冷启动新 build 目录」** — 浪费 ~75s+ FetchContent 时间，对 CP 自审是非必要的开销
❌ **「修改 FETCHCONTENT_SOURCE_DIR_<dep> 而非 FETCHCONTENT_BASE_DIR」** — 需要为每个依赖单独设置，繁琐且易错
❌ **「软链 build-off/_deps -> build/_deps」** — 系统级 hack，CMake 不保证幂等行为，可能引入 generator 文件冲突

### 数据点

- TASK-20260503-05（首次实证）— DEVTOOL=OFF 1082→1087 baseline 验证，cmake configure 1.8s vs 75s+ 冷启动 → 总 baseline 验证 ~70s vs ~135s = **节省 ~65s（48% 加速）**
- 后续可应用场景：CP1/CP2 多 config 验证 / SDL2=ON 验证 / 任何「同依赖集合 + 不同 cmake option」次配置

### 与历史模式的关联

- **延伸**：`baseline 阻塞 hotfix 分离协议`（TASK-20260503-01 沉淀）+ R12 工具链激进升级风险登记 — 同属「baseline 验证基础设施成熟化」家族
- **互补**：「Phase 0 audit 预跑模式」（TASK-20260503-02 沉淀）— Phase 0 阶段预跑 audit 减负 build；本 SOP 是 build 阶段次配置 baseline 验证基础设施减负
...
## 调参/调阈值/调时间常量类子任务的 5min baseline smoke 实证规则（TASK-20260503-03 反思 §6 #2 — 新模式 / 反复模式 #1 适用范围扩展）

### 背景

反复模式 #1「前置依赖/环境/API 能力未验证」第 9 次命中（TASK-20260503-03 P1）暴露**新子维度**：现有实现 runtime 行为假设。VAN/plan 阶段 grep 看到 API 存在（`set_pipeline_hooks` C ABI + `OnFrame` 调用链 + `update_manager_->Update()`），假设「调 `VX_HELLO_DEVTOOL_AUTOQUIT_MS` 200→600ms = 自然多帧」直接进入 build；实测 autoquit 多大都是 frames=1 — `UpdateManager::Update()` 第 17 行 `if (!dirty_) return;` 短路 + `dirty_` 仅 `transition_mgr_.HasActive()` 才 rearm 是 grep 不能发现的 runtime 机制。

历史命中维度（前 8 次）主要是「依赖包/编译器/工具链/外部 API/环境工具是否存在」，VAN/plan 已成熟检查器；**runtime 行为假设**是新维度，需要专项规则。

### 模式核心

任何 plan §3 子任务涉及「**调 X = 获得 Y**」类**线性外推假设**且 X→Y 因果路径未在 grep 之外做实证 → 必须做 5min baseline smoke 实证或核心 runtime 行为源码 Read 确认。

**触发情形清单**：

| 情形 | 反例（本任务） | 应做 |
|---|---|---|
| 调 ctest 时间常量假设可获得多帧 | autoquit 200→600ms = ~36 frames | 跑一次现有 baseline 看实测 frames=1 → 假设否决 |
| 调 PASS_REGULAR_EXPRESSION 假设可收紧测试 | `frames=[1-9][0-9]*` → `frames=([3-9]\|[1-9][0-9]+)` | Read 触发链确认 ≥3 frames 在静态场景下机制可达 |
| 调 source code magic number 假设调整 runtime 行为 | 调 update_manager 内部某个常量 = 改帧率 | Read 该常量的全部使用点 + 其他依赖路径 |
| 调阈值假设线性外推可获得 N 倍效果 | 调 cache size 2× = 2× hit rate | 跑现有 baseline + 真实 workload 实测 |

### 协议（plan §0 必填两步择一）

**实证路径（推荐 — 5 min 投入）：**
1. 跑一次现有 baseline 命令，记录实测输出到 plan
2. 实证记录格式：「实测命令：[命令] / 实测输出：[输出] / 假设是否成立：✅/❌」

**Read 路径（替代 — 适用 baseline 不可跑情形）：**
1. 若 baseline 依赖大型 reconfigure / 真实硬件 / 用户交互等无法快速跑
2. Read 假设涉及的核心 runtime 行为源码（不只 grep API 存在性）
3. Read 范围：从假设的「触发源」到「目标行为」的完整调用链中的关键短路 / 守卫 / 状态机分支

**两步都不可执行时**：plan §3 子任务必须显式标注「**未验证假设 — 实施失败概率高**」+ 降级为「探针式实施」（先做最小验证子任务，PASS 后再做主体）。

### 关键设计点

1. **grep 仅查存在性 ≠ 验证 runtime 行为** — 反复模式 #1 旧 mental model 局限
2. **「调 X = 获得 Y」线性外推是危险信号** — 任何此类假设必须实证 / Read，不能直接进入实施
3. **5 min 投入 vs 30-60 min 失败回退代价** — ROI 8-16×，几乎总是值得
4. **failure-as-data 是补救机制，不是替代实证** — 即使有完善的失败回退流程，前置实证仍然 ROI 更高

### 反模式

- ❌ plan §0 grep 仅查 API/调用链存在性，不查 runtime 行为机制
- ❌ 假设「调 autoquit ms 多大 = 自然多帧」「调 timeout 大 = 测试更稳定」「调阈值更宽松 = 更多测试 PASS」类**线性外推**
- ❌ 把 baseline 跑过 1 次（如「frames=1 hud_visible=1 旧 PASS」）当作假设可成立证据
- ❌ 「反正失败可以回退 + 拆细化为 P3 候选」心态 — 失败回退代价（~30-60 min + 用户决策路径 + plan 调整）远高于 5 min 实证
...
- **TASK-20260503-03 P1 反例**：实施失败 ~12 min（手测 ~5 + 根因定位 ~2 + 回退 ~1 + AskQuestion ~5 + plan 调整 ~5）— **反复模式 #1 命中代价基线**入库
- **TASK-20260503-03 P3 task #3 正例**：VAN 阶段 audit 预跑（grep 5 处 + Read 上下文判定 5 次）→ 假设「8 处反模式」修正为「2 处反模式」 → build 阶段 0 浪费 / 估时 ×0.20

### 数据点

- TASK-20260503-03（首次实证 — 反复模式 #1 第 9 次命中升级 / 12 min 失败回退代价基线 / P3 audit 预跑 ×0.20 估时正例）
- 模式成熟度：**single-evidence**（待第 2/3 任务实证升级到 dual/triple-evidence）

### 交叉引用

- `.cursor/rules/skills/writing-plans.mdc`「调参/调阈值/调时间常量类子任务前置 baseline smoke 实证（必填）」段
- `memory-bank/techContext.md`「Veloxa 引擎核心机制注意点 — frame hooks dirty_ 短路依赖」段

---

## DevTool isolated JSRuntime 协议（TASK-20260503-04 反思 §5 #3 — 新沉淀 / spec §11.1 落地范本）

> **背景**：spec §11.1 强制要求 DevTool Console 用 isolated JSRuntime（独立于 target Document JS 上下文）+ capability allowlist（默认仅 read-only DOM access，禁 file / network / Hot Reload trigger）双层防御，避免 user-untrusted REPL eval 污染 target Document scope 或调用 mutator API。TASK-20260503-04 实施后沉淀为完整协议，供后续 DevTool 子系统（如 JS Debugger backend）复用。

### 1. 三层 JSRuntime 架构（Application 持有）

| 层 | 实例 | 用途 | capability 范围 |
|---|---|---|---|
| **L1 主应用层** | `script_engine_` | target Document JS 执行（用户 page 的 inline `<script>` + onclick 等）| 完整 DOM mutator + 标准 Web API（受 Veloxa 当前实现子集限制）|
| **L2 DevTool UI 层** | `devtool_script_engine_` | DevTool 自身 UI 的 inline JS（inspector_panel.js + console_panel.js）| 全 DOM mutator（针对 DevTool 自己的 Document）+ host endpoint 注册（query-only：dom_json/perf_stats/hot_reload_status/console_log_drain/console_eval）|
| **L3 Console REPL 层**（V1=B 新增） | `console_script_engine_` | user-untrusted REPL eval（vx_view_eval_console source）| **严格隔离**：仅 console.log/error/warn — 不调 DomBindings::Bind / 不暴露 dom_json/perf_stats/eval/drain / 不暴露 file/network/Hot Reload trigger |

**关键不变量**：每层实例独立 `JSRuntime + JSContext`（QuickjsEngine 自包含），相互不共享 GC heap / 不共享 callable / 不共享 opaque pointer。

### 2. lifecycle 时序图（4 阶段）

```
Application::LoadDevtoolDocument()                 Application::UnloadDevtoolDocument()
─────────────────────────────────────              ──────────────────────────────────────
1. devtool_script_engine_->Init()                  4'. UnregisterConsoleBindings (隐式)
2. console_script_engine_->Init()                  3'. console_log_buffer_.reset()
3. console_log_buffer_ = make_unique<...>()        2'. console_script_engine_.reset()
4. devtool_dom_bindings_ = make_unique<DomBindings>(devtool_doc) {
     SetConsoleEngine(console_script_engine_.get())     ← 反序释放（防 dangling）
     SetConsoleLogBuffer(console_log_buffer_.get())
   }
5. RegisterDevtoolBindings(devtool_ctx, devtool_dom_bindings_.get()) {
     注册既有 query-only：dom_json / perf_stats / hot_reload_status
     额外注册：vx_console_eval / vx_devtool_get_console_log_drain  ← 通过 DomBindings 取 ConsoleEngine + ConsoleLogBuffer
   }
6. RegisterConsoleBindings(console_ctx, console_log_buffer_.get()) {
     仅注册：console.log / console.error / console.warn
     **不注册** drain/eval（防 user JS 探测自己的 buffer 或递归 eval）
   }
                                                   1'. devtool_script_engine_.reset()  （由其他 unload 步骤）
```

**反序释放铁律**：销毁顺序与构造顺序相反（4'→3'→2'→1'），避免 dangling JSContext callback / dangling host pointer。

### 3. dual-ctx wiring SOP — DomBindings host data channel 模式（plan-fact reconcile #1 修正）

**问题**：panel JS（如 console_panel.js）由 `LoadDevtoolDocument` 内联到 inspector_panel.html → 走 **devtool_script_engine_** ctx 运行，但 user-untrusted REPL eval 应该跑在 **console_script_engine_** ctx（隔离）。两个 ctx 不同步 → panel JS 无法直接 call 在 console_ctx 注册的 host endpoint（drain/eval）。

**path 1（错误 / 反模式）**：在 console_ctx 注册 drain/eval → panel JS（在 devtool_ctx）找不到这两个函数 → wiring 失败。

**path 2（正确 / 推荐）**：通过 DomBindings 作为 host data channel，让 RegisterDevtoolBindings 在 devtool_ctx 上额外注册 drain/eval（这两个函数从 DomBindings 取 ConsoleEngine / ConsoleLogBuffer 指针）。具体：

```cpp
// dom_bindings.h
class DomBindings {
 public:
  // 既有 setter（HotReloadManager / PerfOverlay 范式）
  void SetHotReloadManager(HotReloadManager* manager);

  // 新增（TASK-20260503-04 D.4）
 ...
 private:
  // InstanceData 持指针（不持有所有权）
  struct InstanceData {
    HotReloadManager* hot_reload_manager = nullptr;
    vx::devtool::console::ConsoleEngine* console_engine = nullptr;
    vx::devtool::console::ConsoleLogBuffer* console_log_buffer = nullptr;
  };
};

// dom_bindings.cc — RegisterDevtoolBindings（在 devtool_ctx 上）
JSValue VxConsoleEval(JSContext* ctx, ...) {
  auto* bindings = static_cast<DomBindings*>(JS_GetContextOpaque(ctx));
  if (!bindings || !bindings->console_engine()) return JS_UNDEFINED;
  // 走 console_script_engine_ 跑 user JS（隔离 ctx）
  auto result = bindings->console_engine()->EvalGlobal(source, "<console>");
  ...
}

JSValue VxDevtoolGetConsoleLogDrain(JSContext* ctx, ...) {
  auto* bindings = static_cast<DomBindings*>(JS_GetContextOpaque(ctx));
  if (!bindings || !bindings->console_log_buffer()) return JS_NewString(ctx, "{}");
  return JS_NewString(ctx, bindings->console_log_buffer()->DrainAsJson().c_str());
}
```

**收益**：
- 0 重写代价（复用既有 SetHotReloadManager / SetPerfOverlay 范式）
- 0 opaque slot 冲突（DomBindings 已占 devtool_ctx 的 opaque slot，dual-channel 共用）
- console_script_engine_ 仍**严格隔离**（仅暴露 console.log/error/warn — 因为 user JS 不应能 query 自己的 buffer 或递归 eval）

### 4. capability allowlist 反向探针单测协议

**强制要求**：每个 isolated runtime 都必须有反向探针单测，验证关键 mutator / sensitive API 在该 runtime ctx 下**不可达**：

```cpp
// tests/devtool/console/console_bindings_test.cc — CapabilityAllowlistReverseProbe
TEST_F(RegisterConsoleBindingsTest, CapabilityAllowlistReverseProbe) {
  // 反向探针：8 项 mutator / sensitive API 在 console ctx 下应 undefined
  ...
  for (auto& probe : probes) {
    auto result = engine->EvalGlobal(probe, "<probe>");
    ASSERT_TRUE(result.ok());
    EXPECT_EQ(result.value(), "undefined") << "Capability leak: " << probe;
  }
}
```

### 5. 复用清单（后续 DevTool 子系统适用）

如要新增 DevTool 子系统（如 JS Debugger backend / Network Inspector / Memory Profiler），按以下清单实施：

- [ ] 决定该子系统是否需要独立 JSRuntime 实例（如涉及 user-untrusted eval → 必须独立 / 仅 host query → 可复用 devtool_script_engine_）
- [ ] 如独立：在 Application::LoadDevtoolDocument 加 lifecycle（按 §2 时序）
- [ ] capability allowlist：列出该 runtime 应**仅**暴露的 host endpoint（默认 query-only / 禁 mutator）
- [ ] dual-ctx wiring：如 panel JS 需访问 host endpoint，按 §3 path 2（DomBindings host data channel）实施
- [ ] 反向探针单测：按 §4 模板验证关键 mutator / sensitive API 不可达
- [ ] A14 链接闭包黑名单：在 `tests/smoke/devtool_a14_link_closure.cmake` 加该子系统全部 C++ 符号

### 交叉引用

- `memory-bank/reflection/reflection-TASK-20260503-04.md` §3.1 + §4.1 + §8（plan-fact reconcile #1 + Phase 0 ctx 归属 audit + T1 mitigation 5 维度 + capability allowlist 完整性）
- `memory-bank/creative/creative-devtool-console.md` §C2 + §C3（isolated JSRuntime 实施方案 + capability allowlist 全量 API 表）
- `memory-bank/techContext.md` 「TASK-20260503-04 DevTool Phase D · Console JS REPL 实施摘要」段
- `docs/specs/2026-04-30-devtool-design.md` §11.1（spec 强制要求 — 已闭环）
- `veloxa/devtool/console/{console_engine.h, console_engine.cc, console_bindings.h, console_bindings.cc}`（实现源 + 完整代码骨架）
- `veloxa/script/{dom_bindings.h, dom_bindings.cc}`（host data channel 扩展实现）
...
## 蓝图任务规模估算公式（TASK-20260504-01 反思入库 / P1 沉淀 — 防 spec 行数估算偏差）

> **背景**：TASK-20260504-01 plan §B5 估 spec 600-800 行 / 实际 449 行 / 偏差 -25%~-44% / 原因：MVP-scope 主题信息密度比 DevTool 主题紧凑（验收矩阵化 + 附录复用 productContext） — 纯感觉估时方法学不够稳定。

### 公式

| 文档类型 | 行数估算公式 | 偏差容忍 |
|---|---|:-:|
| **spec（蓝图主交付）** | `N 段 × 30-50 行/段 + M 附录 × 30-100 行/附录` | ±20% |
| **plan（蓝图实施计划）** | `K 子任务 × 50-80 行/子任务 + 80-120 行框架（前/后段）` | ±15% |
| **creative（详细设计）** | `S 段 × 30-50 行/段 + 50-100 行方案对比表 + 50-100 行 §安全/systemPatterns/验收` | ±25% |
| **archive（归档汇总）** | `参考 reflection 行数 × 1.0-1.5` | ±20% |
| **reflection（回顾）** | `8 段固定 × 30-50 行/段 = 240-400 行` | ±25% |

### 实证数据

| 任务 | 文档 | 估时 | 实际 | 偏差 | 公式预测 |
|---|---|:-:|:-:|:-:|:-:|
| TASK-20260504-01 | spec | 600-800 | **449** | -25%~-44% | 12 段×35 + 4 附录×30 = **540** ✅ |
| TASK-20260504-01 | plan | 400-500 | **505** | +1% | 5 子任务×60 + 100 框架 = **400** ✅（命中下沿）|
| TASK-20260504-01 | creative-mvp-A | 300-400 | **272** | -10%~-32% | 10 段×30 = **300** ✅（命中下沿）|
| TASK-20260504-01 | creative-mvp-B | 400-500 | **469** | ✅ 准确 | 11 段×40 + 50 安全 = **490** ✅ |
| TASK-20260504-01 | creative-mvp-C | 500-600 | **525** | ✅ 准确 | 11 段×45 + 70 安全 + 50 长期愿景 = **615** ✅（命中上沿）|
| TASK-20260504-01 | reflection | — | **290** | — | 8 段×35 = **280** ✅ |

### 适用条件

- **应用阶段**：plan §B5 / §5 estimating 段
- **触发场景**：Level 3-4 蓝图任务（含 V2=a + V2=b + V2=c 全变体）
- **不适用**：Level 1-2 短任务 / fast fix / 注释类改动

### 反模式

- ❌ 凭感觉估「~600-800 行」无公式依据
- ❌ 估时区间过宽（>50%）失去 estimating 意义
- ❌ 跨任务复制估时不调整公式参数

### 交叉引用

- `memory-bank/reflection/reflection-TASK-20260504-01.md` §1.1 行数对比表 + §3.c 教训 #2
- `memory-bank/archive/archive-TASK-20260504-01.md` §6 实证数据完整版

---

## 超 MVP plus 标识范式（TASK-20260504-01 反思入库 / P2 沉淀 — 防优先级争议）

> **背景**：TASK-20260504-01 spec §10 + §11.3 明确将 DevTool Phase E/F/G + Vulkan + Web Animations + SVG / 完整 ICU 国际化等子系统标识为「**超 MVP / 是 plus 而非 MVP**」，解决了「DevTool 4 件套收口后下一步该 Phase E 还是 R2 补全」的优先级争议。可推广到所有「长期愿景子系统」标识。

### 触发条件

任一项满足时，应在对应 spec / archive 中**显式标注**「超 MVP / plus」：

1. **子系统不是核心目标硬指标** — 不直接对应 [`projectbrief.md` 5 大核心目标](33afb7c) 任一项
2. **MVP-A/B/C 三档完成度评估时不计入** — 仅计入 plus 评估
3. **资源投入巨大且无 MVP 阻塞** — 如 Level 4+ 多 Phase / plan ×0.6 ≥ 30 h
4. **长期愿景或市场需求驱动** — 非短期 ROI 推动
...
| # | 子系统 | 来源 | 量级 |
|:-:|---|---|:-:|
| 1 | [子系统名] | [来源 spec / activeContext] | Level 4 |
```
...
#### 在 productContext 已实现表

不入表（仅 MVP-A/B/C 三档已实现 / 超 MVP plus 不入已实现表 / 防混淆）。

### 实证数据

| 任务 | 标识的「超 MVP plus」子系统 | 解决的优先级争议 |
|---|---|---|
| TASK-20260504-01 | DevTool Phase E (JS Debugger) / Phase F (CDP 远程调试) / Phase G (UI 编辑器) / Vulkan / 完整 Web Animations / SVG / 完整 ICU 国际化 | DevTool 4 件套收口后下一步该 Phase E 还是 R2 补全？ → **答：MVP-B R2 补全优先 / Phase E 是 plus 等用户需求驱动** |

### 反模式

- ❌ 长期愿景子系统标识为「MVP-D / MVP-E」（混淆 MVP 概念 / 应标识为 plus）
- ❌ 「未来工作」段含模糊词如「也许 / 可能 / 后续」（应明确「超 MVP / plus」语义）
- ❌ 已实现表混入 plus 子系统（防混淆 MVP 完成度评估）

### 交叉引用

- `memory-bank/reflection/reflection-TASK-20260504-01.md` §3.a #6（最大战略贡献）
- `docs/specs/2026-05-04-mvp-scope.md` §10 + §11.3 范例
- `README.md` 「项目状态 → 路线图」段「超 MVP-C（plus 而非 MVP）」子段

---

## 跨决策协同度 100% sept-evidence（TASK-20260504-01 反思入库 / 第 8 次连续命中 — sept-evidence 升级）

> **背景**：N 个用户决策（V/C/B 系列 brainstorm + plan 决策）通过 1 次 AskQuestion all_recommended 锁定，0 调整 0 反悔 = 跨决策协同度 100%。本次第 8 次连续命中，已达 sept-evidence 升级阈值，正式入 systemPatterns。

### 实证数据

| # | 任务 | 决策数 | 形式 | 锁定时间 |
|:-:|---|:-:|---|:-:|
| 1 | TASK-20260430-04（DevTool 蓝图）| 11 | V1-V5 + B1-B6 全推荐 1 次 AskQuestion | ~5 min |
| 2 | TASK-20260502-01（DevTool Phase A）| 12 | V/B 全推荐 1 次 AskQuestion | ~5 min |
| 3 | TASK-20260502-02（DevTool Phase B）| 12 | V/B 全推荐 1 次 AskQuestion | ~3 min |
| 4 | TASK-20260503-01（DevTool Phase C）| 10 | V/B 全推荐 1 次 AskQuestion | ~3 min |
| 5 | TASK-20260503-02（工作流元任务）| 13 | V/B 全推荐 1 次 AskQuestion | ~5 min |
| 6 | TASK-20260503-05（QuickJS Interrupt）| 12 | V/B + D8b push-back 全推荐 | ~5 min |
| 7 | TASK-20260503-04（DevTool Phase D）| 12 | C1-C4 + B1-B8 全推荐 1 次 AskQuestion | ~5 min |
| 8 | **TASK-20260504-01**（MVP-scope 蓝图）| **11** | V1+V3+V4+V5 + B1-B7 全推荐 1 次 AskQuestion | **~5 min** |
| **累计** | — | **93 / 93** | — | — |
...
1. **plan 阶段或 brainstorm 阶段抛 ≥ 5 个决策**（V 系列 5+ / B 系列 5+ / C 系列 4+）
2. **每个决策必须给出明确推荐方案 + 1 句理由**（详见 [Checkpoint 推荐默认 + 隐式批准协议](#checkpoint-推荐默认--隐式批准协议task-30-03-反思44--新模式)）
3. **AskQuestion 选项必须包含 all_recommended 一键锁定**（避免用户细分疲劳）
4. **如有不同意见，应在 plan 落盘前抛出 brainstorm 而非默认通过**（否则违反「证据优于断言」）

#### 范式可处理决策规模

| 决策数 | 适用 | 备注 |
|:-:|---|---|
| ≤ 5 | ✅ | 简单 / 无需 AskQuestion 也可逐个询问 |
| 6-10 | ✅ | 推荐 all_recommended + 单选选项 fallback |
| 11-13 | ✅ | **本范式实证最优区间** / 8 次连续命中 |
| 14-15 | ⚠️ | 接近用户决策疲劳上限 / 应考虑拆分 |
| ≥ 16 | ❌ | 应拆分到多个阶段（brainstorm + plan B + creative C 系列）|

### 加速因子

- **VAN/plan 阶段决策时间从 ~30-60 min（逐个询问 + 来回澄清）压缩到 ~5 min（all_recommended 一键锁定）**
- **0 反悔 0 调整 → build/creative 阶段无返工成本**
- **commit body 可直接引用决策表 + Source 段 quad-evidence**

### 反模式

- ❌ 决策推荐方案模糊（「视情况」/「待定」/「灵活处理」）→ 用户无法 all_recommended
- ❌ 决策选项未包含 all_recommended 一键锁定 → 用户必须逐个细看 / 时间浪费
- ❌ 推荐方案无理由 / 1 句话理由太抽象 → 用户无法判断是否同意
- ❌ 决策数 ≥ 16 强行抛 1 次 AskQuestion → 用户决策疲劳 / 错误率上升

### 交叉引用

- `memory-bank/reflection/reflection-TASK-20260504-01.md` §3.a #1 + §3.c #4
