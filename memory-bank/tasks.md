...
- **状态：** 🟢 BUILD R4 完成（#21 LayoutInline LineBox 模型 — VerticalAlign 9 关键字 + length/percent + TextMetrics ABI 兼容拆 ascent/descent + 新建 line_box.h POD + LayoutInline 严格 2-pass vertical-align 重写 + 半-leading + 隐式 strut + LineBox Vector + fit-content width / explicit height 修正 + inline-block atomic 路径；ctest 1029/1029 + 19 新 case + 2 wpt linebox fixture（Wpt006/Wpt007 PASS）+ 同窗口对照 bench Flat/64 mean -3.6% / median +2.65% 远低 ±10%），准备 commit + /reflect
- **创建日期：** 2026-04-26
...
| 2026-04-26 02:55 | 构建·R2 | BUILD R2 完成 [安全相关]；#28 HTML inline style + 完整三件套安全护栏 — `kInlineStyleMaxDeclarationCount = 1000` + `kInlineStyleMaxValueLength = 8 * 1024` 暴露到 `parser.h` namespace（**实证微调**：plan 原 hidden constants 改为可测） + `internal::ContainsBlacklistKeyword(StringView)` 子模块化（**实证微调**：plan 原匿名 namespace 改为暴露 unit test 入口，因 R2.5 反向探针发现 e2e 黑名单测试被 CssParser 自然丢弃 unknown property over-match） + ApplyInlineStyleAttribute 私有方法实现 (T2 8KB cap → blacklist → ParseDeclarationList → T1 1000 cap loop SetInlineDeclaration) + ProcessStartTag style 分支（kStyleAttr ≡ Intern("style") 抬出循环外）；ctest **984/984 PASS** in 2.08s（R1 960 + 24 = inline_style 14 + InlineStyleSecurityConstants 1 + ContainsBlacklistKeyword 8 + e2e 1 InlineStyleAttributePaddingTakesEffect；plan §2 R2.7 估 +8 实测 +24，**实证微调**完整威胁建模需要更多 case）/ Release `-O3 -Werror` 0 warn / linter 0 issue；bench `BM_ParseDeclarationListInline/16` 2018→1947 ns **-3.5%**（首次跑 +10.3% 是 Load 1.03 噪声，sleep 30s 稳态后改善 — TASK-19-13 P1 协议第 2 次自证）/ /32 4085→4015 ns -1.7% / Stylesheet 全在 ±1.6% 内；**RED 反向探针完整循环**：ContainsBlacklistKeyword `return false;` 临时 break → 5/5 match positives 立即 FAIL（3 reject negatives 不受影响）→ 恢复后 8/8 PASS，反向探针真正区分实现 vs placeholder；**3 过程实证收获**：(1) SetInlineDeclaration upsert 语义破坏 cap test 可观测性 — dedup 后 1500 项缩成 1，cap 实证依赖 spec review；(2) CssParser 自然错误恢复 over-match e2e blacklist 测试 — 提取 internal helper 直接测才是真实证（**「测试该测什么」边界发现**）；(3) StringView ctor 非 constexpr → `constexpr [] = ` 编译失败 + unused-function `-Werror` 卡死 → 改 `const StringView []` + `[[maybe_unused]]` 容忍临时禁用；plan × 0.6 第 11 数据点 **0.56×**（plan 90 min / 实测 ~50 min，与 R1 0.5× 衔接形成「单文件 D2 注入点」连续 2 数据点 0.5-0.56×；中位 0.28× 偏高因 R2 含 8 直接测试 + 1 e2e 比 R1 多） |
